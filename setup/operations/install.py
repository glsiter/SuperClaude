...
Examples:
  ...
  SuperClaude.py install --verbose --force        # Verbose with force mode
    ...
    parser.add_argument(
        "--minimal", action="store_true", help="Minimal installation (core only)"
    )
...
def validate_system_requirements(
    ...
    try:
        ...
        if success:
            ...
            print(
                "  Run 'SuperClaude.py install --diagnose' for detailed system diagnostics"
            )
...
def get_components_to_install(
    args: argparse.Namespace, registry: ComponentRegistry, config_manager: ConfigManager
) -> Optional[List[str]]:
...
def interactive_component_selection(
    registry: ComponentRegistry, config_manager: ConfigManager
) -> Optional[List[str]]:
    ...
    try:
        ...
        for component_name in available_components:
            ...
            if metadata:
                ...
                menu_options.append(
                    f"{component_name} - Component description unavailable"
                )
        ...
        if choice == -1:  # Cancelled
            ...
            component_menu = Menu(
                "Select components to install:", menu_options, multi_select=True
            )
