...
class MCPComponent(Component):
    ...
    def _install_mcp_server(
        ...
        try:
            ...
            if config.get("dry_run", False):
                self.logger.info(
                    f"Would install MCP server (user scope): claude mcp add -s user {server_name} {command}"
                )
                return True

            self.logger.debug(
                f"Running: claude mcp add -s user {server_name} {command}"
            )
    ...
    def install(self, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if failed_servers:
                self.logger.warning(
                    f"Some MCP servers failed to install: {failed_servers}"
                )
                self.logger.success(
